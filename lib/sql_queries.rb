...
def selects_the_titles_of_all_projects_and_their_pledge_amounts_alphabetized_by_title
  "SELECT projects.title, SUM(pledges.amount) FROM projects LEFT OUTER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title ORDER BY title ASC;"
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
  "SELECT users.name, users.age, SUM(pledges.amount) FROM pledges LEFT OUTER JOIN users ON users.id = pledges.user_id GROUP BY users.name ORDER BY name ASC;"
end
